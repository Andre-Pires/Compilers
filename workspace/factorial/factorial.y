...
declaracao  : pub cons tipo ptr IDENTIF init ';'            { IDnew($1->info+$2->info+$3->info+$4->info, $5, 0); declare($5, $6->info); /* alterar o declare para não fazer IDnew e usar o que vem antes dele na declaracao */
            ...
            | pub cons tipo ptr IDENTIF ';'                 { $$=uniNode(DECL, strNode(IDENTIF, $5)); IDnew($1->info+$2->info+$3->info+$4->info, $5, 0); declare($5, 0);} /* $1->info+$2->info+$3->info+$4->info*/
...
expressao : INT                                       { $$ = intNode(INT, $1); $$->info = 1; }
          ...
          | left_value                                { $$ = $1;}
          | IDENTIF '(' expressoes ')'                { int n; n = IDfind($1, 0) & (0x7); if(n != -1 && n == 4) {$$ = binNode(CALL2, strNode(IDENTIF, $1), $3); $$->info = n;} else {$$ = binNode(CALL, strNode(IDENTIF, $1), $3); $$->info = IDsearch($1, 0, IDlevel(), 0) & (0x7);}}    /* fazer o search no nivel 0 para nao confundir a funcao com a variavel */
