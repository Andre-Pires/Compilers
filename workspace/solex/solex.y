...
expressao : LIT_NUM             { $$ = intNode(LIT_NUM, $1); }
    | LIT_STR             { $$ = strNode(LIT_STR, $1); }
    | left_value              { $$ = $1; }
    | ID '(' expressoes ')'           { $$ = subNode(CALL, 2, strNode(ID, $1), $3); }
    | '(' expressao ')'           { $$ = $2; }
    | left_value '=' expressao          { $$ = subNode('=', 2, $1, $3); }
    | '-' expressao %prec UMINUS          { $$ = subNode(UMINUS, 1, $2); }
    | INC left_value            { $$ = subNode(INC, 2, nilNode(END), $2); }
    | DEC left_value            { $$ = subNode(DEC, 2, nilNode(END), $2); }
    | left_value INC            { $$ = subNode(INC, 2, $1, nilNode(END)); }
    | left_value DEC            { $$ = subNode(DEC, 2, $1, nilNode(END)); }
    | expressao POW expressao         { $$ = subNode(POW, 2, $1, $3); }
    | expressao '*' expressao         { $$ = subNode('*', 2, $1, $3); }
    | expressao '/' expressao         { $$ = subNode('/', 2, $1, $3); }
    | expressao '%' expressao         { $$ = subNode('%', 2, $1, $3); }
    | expressao '+' expressao         { $$ = subNode('+', 2, $1, $3); }
    | expressao '-' expressao         { $$ = subNode('-', 2, $1, $3); }
    | expressao '>' expressao         { $$ = subNode('>', 2, $1, $3); }
    | expressao '<' expressao         { $$ = subNode('<', 2, $1, $3); }
    | expressao EQ expressao          { $$ = subNode(EQ, 2, $1, $3); }
    | expressao NE expressao          { $$ = subNode(NE, 2, $1, $3); }
    | expressao GE expressao          { $$ = subNode(GE, 2, $1, $3); }
    | expressao LE expressao          { $$ = subNode(LE, 2, $1, $3); }
    | expressao '&' expressao         { $$ = subNode('&', 2, $1, $3); }         
    | expressao '|' expressao         { $$ = subNode('|', 2, $1, $3); }
    | '!' expressao             { $$ = subNode('!', 1, $2); }
    | '&' left_value %prec ADDR         { $$ = subNode(ADDR, 1, $2); }  
    | '*' left_value %prec LOAD         { $$ = subNode(LOAD, 1, $2); }        
    ;
